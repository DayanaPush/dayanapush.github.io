...
***************************************************************************** */
    ...
    const modals = () => {
        ...
        bindModal(".header__action-button", ".popup", ".popup .popup__close");
    };
    const modules_modals = modals;
    const form_forms = document.forms;
    if (form_forms.length) for (const form of form_forms) form.addEventListener("submit", formSubmitAction);
    async function formSubmitAction(e) {
        ...
        if (response.ok) {
            ...
            setTimeout(hidePopup, 3e3);
        } else {
            showPopupError();
            closeModal.style.display = "none";
            setTimeout(hidePopupError, 3e3);
            form.classList.remove("form-sending");
            form.reset();
        }
    }
    const popup = document.querySelector(".popup__submit");
        ...
        elementsToAnimate.forEach((element => {
            ...
            const animateText = () => {
                ...
                letters.forEach(((letter, index) => {
                    ...
                    }), index * element.dataset.speed || 100);
                }));
            };
            const handleScroll = () => {
